...
function RegisterFruits() {
  ...
  const fruitAlreadyExists = async (name: string): Promise<boolean> => {
    ...
    if (error) {
      console.error("Erro ao verificar fruta:", error);
      return false;
    }

    return fruits && fruits.length > 0;
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (alreadyExists) {
      setErrors((prevErrors) => ({
        ...prevErrors,
        name: "Já existe uma fruta cadastrada com esse nome.",
      }));
      return;
    }

    try {
      // Gerar um ID único baseado em timestamp + número aleatório
      ...
      const { error } = await supabase.from("fruits").insert([FruitWithId]);

      if (!error) {
        setSucesso("Fruta cadastrada com sucesso!");
        console.log("Fruta cadastrada:", FruitWithId);
      ...
      <form onSubmit={handleSubmit} className={style.form}>
        ...
        <div className={style.inputGroup}>
          ...
          <input
            ...
            value={fruit.image}
            onChange={(e) => {
              setFruit({ ...fruit, image: e.target.value });
            }}
